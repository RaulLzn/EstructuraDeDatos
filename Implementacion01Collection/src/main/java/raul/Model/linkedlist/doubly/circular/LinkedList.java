package raul.Model.linkedlist.doubly.circular;
...
public class LinkedList<E> extends AbstractList<E> {
    ...
    public boolean add(E element) {
        try {
            if (head == null) {
                ...
                LinkedNode<E> newNode = new LinkedNode<>(element, head, head.getPrevious());
    ...
    public boolean replace(Collection<E> collection, Collection<E> newCollection, Predicate<E> comparator) {
        try {
            ...
            while (iterator.hasNext()) {
                replace(iterator.next(), newIterator.next(), comparator);
            }
            return true;
        } catch (Exception e) {
            Logger.getLogger(this.getClass().getName()).log(Level.WARNING, e.getMessage(), e);
